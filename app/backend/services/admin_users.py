...
# ------------------ Service Layer ------------------
class Admin_usersService:
    """Service layer for Admin_users operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Admin_users]:
        """Create a new admin_users"""
        try:
            obj = Admin_users(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created admin_users with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating admin_users: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Admin_users]:
        """Get admin_users by ID"""
        try:
            query = select(Admin_users).where(Admin_users.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching admin_users {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of admin_userss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Admin_users, field):
                        column = getattr(Admin_users, field)
            ...
            stmt = select(Admin_users, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Admin_users, field_name):
                        stmt = stmt.order_by(getattr(Admin_users, field_name).desc())
                else:
                    if hasattr(Admin_users, sort):
                        stmt = stmt.order_by(getattr(Admin_users, sort))
            else:
                stmt = stmt.order_by(Admin_users.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Admin_users)
                for condition in conditions:
            ...
            logger.error(f"Error fetching admin_users list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Admin_users]:
        """Update admin_users"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Admin_users {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated admin_users {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating admin_users {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Admin_users {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted admin_users {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting admin_users {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Admin_users]:
        """Get admin_users by any field"""
        try:
            if not hasattr(Admin_users, field_name):
                raise ValueError(f"Field {field_name} does not exist on Admin_users")
            column = getattr(Admin_users, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Admin_users).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching admin_users by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Admin_users]:
        """Get list of admin_userss filtered by field"""
        try:
            if not hasattr(Admin_users, field_name):
                raise ValueError(f"Field {field_name} does not exist on Admin_users")
            column = getattr(Admin_users, field_name)
            ...
            logger.error(f"Error fetching admin_userss by {field_name}: {str(e)}")
            raise