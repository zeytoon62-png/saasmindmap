...
# ------------------ Service Layer ------------------
class Report_runsService:
    """Service layer for Report_runs operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Report_runs]:
        """Create a new report_runs"""
        try:
            obj = Report_runs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created report_runs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating report_runs: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Report_runs]:
        """Get report_runs by ID"""
        try:
            query = select(Report_runs).where(Report_runs.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching report_runs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of report_runss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Report_runs, field):
                        column = getattr(Report_runs, field)
            ...
            stmt = select(Report_runs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Report_runs, field_name):
                        stmt = stmt.order_by(getattr(Report_runs, field_name).desc())
                else:
                    if hasattr(Report_runs, sort):
                        stmt = stmt.order_by(getattr(Report_runs, sort))
            else:
                stmt = stmt.order_by(Report_runs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Report_runs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching report_runs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Report_runs]:
        """Update report_runs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Report_runs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated report_runs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating report_runs {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Report_runs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted report_runs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting report_runs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Report_runs]:
        """Get report_runs by any field"""
        try:
            if not hasattr(Report_runs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Report_runs")
            column = getattr(Report_runs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Report_runs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching report_runs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Report_runs]:
        """Get list of report_runss filtered by field"""
        try:
            if not hasattr(Report_runs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Report_runs")
            column = getattr(Report_runs, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Report_runs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching report_runss by {field_name}: {str(e)}")
            raise