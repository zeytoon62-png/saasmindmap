...
# ------------------ Service Layer ------------------
class Crypto_walletsService:
    """Service layer for Crypto_wallets operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Crypto_wallets]:
        """Create a new crypto_wallets"""
        try:
            obj = Crypto_wallets(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created crypto_wallets with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating crypto_wallets: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Crypto_wallets]:
        """Get crypto_wallets by ID"""
        try:
            query = select(Crypto_wallets).where(Crypto_wallets.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching crypto_wallets {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of crypto_walletss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Crypto_wallets, field):
                        column = getattr(Crypto_wallets, field)
            ...
            stmt = select(Crypto_wallets, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Crypto_wallets, field_name):
                        stmt = stmt.order_by(getattr(Crypto_wallets, field_name).desc())
                else:
                    if hasattr(Crypto_wallets, sort):
                        stmt = stmt.order_by(getattr(Crypto_wallets, sort))
            else:
                stmt = stmt.order_by(Crypto_wallets.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Crypto_wallets)
                for condition in conditions:
            ...
            logger.error(f"Error fetching crypto_wallets list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Crypto_wallets]:
        """Update crypto_wallets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Crypto_wallets {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated crypto_wallets {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating crypto_wallets {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete crypto_wallets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Crypto_wallets {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted crypto_wallets {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting crypto_wallets {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Crypto_wallets]:
        """Get crypto_wallets by any field"""
        try:
            if not hasattr(Crypto_wallets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Crypto_wallets")
            column = getattr(Crypto_wallets, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Crypto_wallets).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching crypto_wallets by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Crypto_wallets]:
        """Get list of crypto_walletss filtered by field"""
        try:
            if not hasattr(Crypto_wallets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Crypto_wallets")
            column = getattr(Crypto_wallets, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Crypto_wallets.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching crypto_walletss by {field_name}: {str(e)}")
            raise