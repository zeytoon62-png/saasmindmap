...
class FeedbacksService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Feedbacks, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Feedbacks, field_name):
                        stmt = stmt.order_by(getattr(Feedbacks, field_name).desc())
                else:
                    if hasattr(Feedbacks, sort):
                        stmt = stmt.order_by(getattr(Feedbacks, sort))
            else:
                stmt = stmt.order_by(Feedbacks.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Feedbacks)
                for condition in conditions:
