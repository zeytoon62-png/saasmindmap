...
class Email_logsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Email_logs]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Email_logs]:
        """Get email_logs by ID"""
        try:
    ...
    async def get_list(
        ...
        """Get paginated list of email_logss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Email_logs, field):
                        column = getattr(Email_logs, field)
            ...
            stmt = select(Email_logs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Email_logs, field_name):
                        stmt = stmt.order_by(getattr(Email_logs, field_name).desc())
                else:
                    if hasattr(Email_logs, sort):
                        stmt = stmt.order_by(getattr(Email_logs, sort))
            else:
                stmt = stmt.order_by(Email_logs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Email_logs)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Email_logs]:
        """Update email_logs"""
        try:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Email_logs]:
        ...
        try:
            ...
            column = getattr(Email_logs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Email_logs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
